...
export class AppComponent implements OnInit{
  ...
  newMap(){
      ...
      var lastDate = new Date(this.covid_total_timeline[this.covid_total_timeline.length - 1].date);
      ...
      var countryIndexMap = {};
      var list = this.covid_world_timeline[this.covid_world_timeline.length - 1].list;
      ...
      for (var i = 0; i < this.covid_total_timeline.length; i++) {
        var di = this.covid_total_timeline[i];
      ...
      function getSlideData(index) {
        var data = this.covid_world_timeline[index];
      ...
      var mapData = JSON.parse(JSON.stringify(this.covid_world_timeline[this.covid_world_timeline.length - 1].list));
      ...
      for (var i = 0; i < mapData.length; i++) {
        var di = mapData[i];
        mapData.active = di.confirmed - di.recovered - di.deaths
      ...
      container.tooltip.fontSize = "10px";
      ...
      imageTemplate.adapter.add("tooltipY", function(tooltipY, target) {
        // console.log(-target.children.getIndex(0) + 'line 2637')
      ...
      filterSlider.events.on("rangechanged", function() {
        ...
        if (!isNaN(maxValue) && bubbleSeries.inited) {
          ...
          console.log(maxValue +  'maaxVal')
          circle.clones.each(function(clone) {
            console.log('line 2963 '+ clone.dataItem)
            console.log(clone.dataItem)
            if (clone.dataItem.values.value.workingValue > maxValue) {
              clone.dataItem.hide();
            }
            else {
              clone.dataItem.show();
      ...
      function addButton(name, color) {
        ...
        button.icon = circle;
        button.label.text = capitalizeFirstLetter(name) ;
      ...
      function changeDataType(name) {
        ...
        if(currentType === "active"){
          bubbleSeries.dataItems.each(function(dataItem) {
            dataItem.setValue("value", dataItem.dataContext["confirmed"]- dataItem.dataContext["deaths"]- dataItem.dataContext["recovered"]);
          })
        }
        else{
          bubbleSeries.dataItems.each(function(dataItem) {
        ...
        bubbleSeries.mapImages.template.tooltipText = "[bold]{id}: {value}[/] [font-size:10px]\n" + currentTypeName;
      ...
      function setCountryData(countryIndex) {
        ...
        for (var i = 0; i < lineChart.data.length; i++) {
          var di = this.covid_world_timeline[i].list;
      ...
      function updateTotals(index) {
        if (!isNaN(index)) {
          var di = this.covid_total_timeline[index];
    ...
    mobileMap(){
      ...
      // main container
      // https://www.amcharts.com/docs/v4/concepts/svg-engine/containers/
      var container = am4core.create("chartmob", am4core.Container);
      container.width = am4core.percent(100);
      container.height = am4core.percent(100);
      ...
      container.tooltip.background.fill = am4core.color("#000000");
      container.tooltip.background.stroke = activeColor;
      container.tooltip.fontSize = "3px";
      container.tooltip.getFillFromObject = false;
      container.tooltip.getStrokeFromObject = false;
      ...
      var buttonsAndChartContainer = container.createChild(am4core.Container);
      buttonsAndChartContainer.layout = "vertical";
      buttonsAndChartContainer.height = am4core.percent(60); // make this bigger if you want more space for the chart
      buttonsAndChartContainer.width = am4core.percent(100);
      buttonsAndChartContainer.valign = "bottom";
      ...
      dateAxis.renderer.grid.template.strokeOpacity = 0.25;
      dateAxis.max = lastDate.getTime() + am4core.time.getDuration("day", 5);
      dateAxis.tooltip.label.fontSize = "3px";
      dateAxis.tooltip.background.fill = activeColor;
      dateAxis.tooltip.background.stroke = activeColor;
      dateAxis.renderer.labels.template.fill = am4core.color("#ffffff");
